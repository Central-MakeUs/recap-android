...
- [ ] 2026-07-15 - 설정 하위 화면(계정 관리, 문의하기, 이용 안내)
  - Context: 설정 top-level UI/명칭과 Gradle 모듈은 `:feature:settings`로 통일됨. 계정 관리·문의하기는 stub, 이용 안내는 임시로 업로드 안내 화면으로 연결
  - Next: 계정 관리/문의하기/이용 안내 전용 화면 스펙 후 구현
  - Handoff: not started
