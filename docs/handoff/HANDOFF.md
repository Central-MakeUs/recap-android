...
- [ ] `settings.gradle.kts`에 `:core:*`, `:feature:*` 모듈이 포함되어 있다.
- [ ] `:app`은 app shell과 navigation composition만 직접 소유하고, core/feature 구현 코드는 각 모듈로 이동되어 있다.
- [ ] feature 모듈은 앱 모듈의 `R` 또는 `BuildConfig`를 import하지 않는다.
- [ ] 기존 화면의 `UiState`/`Action`/`ViewModel` UDF 계약이 유지된다.
- [ ] debug build가 성공한다.
